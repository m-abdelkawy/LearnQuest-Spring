...
public class StudentControllerTest {
    ...
    void testGetOneRequestParamHTMLError() {
        ...
        HttpClientErrorException ex = Assertions.assertThrows(HttpClientErrorException.class, () -> {
            ResponseEntity<String> responseEntity = new RestTemplate()
                    .exchange(BASE_URL + "/college/student/single?if={id}", HttpMethod.GET, new HttpEntity<>(headers), String.class, 4);
        });
    ...
    @Test
    void testGetOneRequestParamHTMLError2() {
        headers.set("accept", "application/html");
        Assertions.assertThrows(HttpClientErrorException.class, () -> {
            ResponseEntity<String> responseEntity = new RestTemplate()
                    .exchange(BASE_URL + "/college/student/single?id={id}", HttpMethod.GET, new HttpEntity(headers), String.class, 3);

            // wrong way to test -> unreachable code
            assertThat(responseEntity.getStatusCode(), equalTo(HttpStatus.BAD_REQUEST));
        });
    }
}
